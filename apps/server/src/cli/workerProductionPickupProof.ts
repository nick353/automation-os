...
const databaseConfigured = Boolean(process.env.AUTOMATION_OS_DATABASE_URL || process.env.DATABASE_URL || process.env.POSTGRES_URI || process.env.POSTGRES_CONNECTION_STRING);
...
if (!databaseConfigured) {
  finish({
    ...
    nextAction: "ローカルshellに本番PostgreSQLのDATABASE_URL、AUTOMATION_OS_DATABASE_URL、POSTGRES_URI、またはlinked serviceのPOSTGRES_CONNECTION_STRINGを設定してから再実行してください。",
...
const childDatabaseUrl = process.env.AUTOMATION_OS_DATABASE_URL ?? process.env.DATABASE_URL ?? process.env.POSTGRES_URI ?? process.env.POSTGRES_CONNECTION_STRING;
