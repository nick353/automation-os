...
import { getResearchPlan, getResearchPlanAsync, type ResearchPlanSnapshot, type ResearchSourceKey } from "./researchPlanner.js";
...
/**
 * Async counterpart for the Postgres scheduler/worker boundary. The
 * scheduler child must not call the legacy sync SQL helpers because those
 * helpers spawn a second Node process with execFileSync for every query.
 * Keep the transaction and its optimistic lineage checks identical to the
 * synchronous path used by older local callers.
 */
export async function commitResearchPlanStartedAtomicAsync(plan: ResearchPlanSnapshot, runId: string): Promise<ResearchPlanSnapshot> {
  const companyId = requireCompanyId(plan);
  const currentRun = await readScopedRunAsync(runId, companyId);
  const currentRunMetadata = parseJson<Record<string, unknown>>(currentRun.metadata_json, {});
  const preparedRun = currentRun.status === "preparing"
    && currentRunMetadata.research_plan_id === plan.id
    && currentRunMetadata.research_plan_start_phase === "prepared_unlinked";
  ...
  const runUpdate = await buildRunBoundaryUpdateAsync({
    runId,
    companyId,
    currentRun,
    plan: updatedPlan,
  ...
  if (preparedRun) steps.push(...await buildPreparedRunActivationStepsAsync(runId, companyId));
  steps.push(runUpdate);
  if (preparedRun && preparedPlan.approvalRequired === true) {
    const approval = createApprovalRequest({
      runId,
  ...
  return await getResearchPlanAsync(plan.id, [companyId]) ?? updatedPlan;
}
...
export async function rollbackPreparedResearchPlanRunAtomicAsync(input: {
  planId: string;
  runId: string;
  companyId: string;
}): Promise<void> {
...
async function buildPreparedRunActivationStepsAsync(runId: string, companyId: string): Promise<SqlTransactionStep[]> {
  const steps = await querySqlAsync<PreparedStepRow>(`
    SELECT id, lane_id, metadata_json FROM run_steps
    WHERE run_id=${sqlValue(runId)} AND company_id=${sqlValue(companyId)}
    ORDER BY id ASC
  `);
...
async function buildRunBoundaryUpdateAsync(input: {
  runId: string;
  companyId: string;
  currentRun: RunRow;
  plan: ResearchPlanSnapshot;
  ...
  metadata: Record<string, unknown>;
  desiredStatus?: string;
}): Promise<SqlTransactionStep> {
  const requiredProofs = requiredResearchPlanProofs(input.plan);
  const approvalBoundarySources = billingRequiredResearchSourceKeys(input.plan);
  const presentProofs = (await querySqlAsync<{ proof_type: string }>(`
    SELECT proof_type FROM proofs
    WHERE run_id=${sqlValue(input.runId)} AND company_id=${sqlValue(input.companyId)}
  `)).map((proof) => proof.proof_type);
  if (input.additionalProofType && !presentProofs.includes(input.additionalProofType)) presentProofs.push(input.additionalProofType);
...
async function readScopedRunAsync(runId: string, companyId: string): Promise<RunRow> {
  const run = (await querySqlAsync<RunRow>(`
    SELECT company_id, status, metadata_json, updated_at FROM runs
    WHERE id=${sqlValue(runId)} AND company_id=${sqlValue(companyId)} LIMIT 1
  `))[0];
