...
await stage("api-readback", async () => {
  ...
  if (readToken) {
    ...
    workflows = extractAutomationRows(rawBody(result.api.registeredWorkflows));
  } else {
    result.api.protectedReadback = {
      status: "blocked",
      attempted: false,
    ...
    fail(readTokenStatus.exactBlocker);
  }
  result.registeredWorkflowReadback = readToken
    ? {
    ...
    : { count: null, ids: [], statuses: [], exact_blocker: readTokenStatus.exactBlocker };
  if (!result.api.health.ok) fail("api_health_unreachable");
  if (readToken && !result.api.dashboard.ok && !result.api.registeredWorkflows.ok) fail("api_mvp_compatibility_unreachable");
  if (readToken && (!result.api.registeredWorkflows.ok || workflows.length === 0)) fail("registered_workflows_empty");
});
...
await stage("create-plan-api-replay", async () => {
  const prompts = {
    ...
    secretOnly: ["OPENAI_API_KEY=fake-test-secret-value 保存だけして。実行しないで。"]
  };
  const cases = {};
