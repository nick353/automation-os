...
Worker receipts use the local Automation OS worker contract. The production stored-secret LaunchAgent explicitly uses `AUTOMATION_OS_PORTABLE_WORKER_MODE=external` for portable registered workflows (求人応募管理, Daily AI, and NisenPrints): it validates the workflow manifest, binds the run, and hands approved work to the canonical Browser Use CLI/MCP adapter with same-run readback and cleanup proof. An explicit `AUTOMATION_OS_PORTABLE_WORKER_MODE=canary` remains available for effect-free binding checks and stops with `portable_external_effects_disabled`. An unset server mode never selects a legacy browser runner. `OPENAI_API_KEY` is not required.
...
`npm run worker:loop:stored` is the local Mac bridge for production control-plane use. Save the same production PostgreSQL `DATABASE_URL` used by Zeabur into Automation OS once, keep the Codex CLI logged in with the ChatGPT subscription, and run the loop on the Mac instead of putting `~/.codex/auth.json` on Zeabur. Prefer the LaunchAgent/start script so its verified `CODEX_CLI_PATH` is inherited; use `AUTOMATION_OS_CHILD_CODEX_BIN` / `AUTOMATION_OS_CODEX_BIN` only for an explicitly verified official CLI override. Do not rely on an unqualified `codex` found through `PATH`. The loop calls `runWorkerOnce()` repeatedly and prints JSON cycle receipts. It does not require `OPENAI_API_KEY`; if API keys are present, the startup receipt says so.
...
The LaunchAgent template `ops/launchd/com.nichikatanaka.automation-os-worker.plist` runs `scripts/start-automation-os-worker.sh`, which calls `npm run worker:loop:stored`. It intentionally contains no database URL or token. Only install or kickstart it after `worker:production-proof:stored` passes; an invalid stored PostgreSQL secret is a hard startup blocker and must not be bypassed with SQLite or a Codex App bridge.
...
Daily AI remains the first registered workflow executor. Current registered Daily AI execution enters the portable external worker and uses the canonical Browser Use CLI stage adapter, with the workflow-owned summary, queue readback, and artifact proof still required. A missing or non-canonical adapter is an exact blocker; it must not fall back to Playwright, direct Chrome, CDP, or the Codex App browser. The spawn has a bounded timeout from `AUTOMATION_OS_DAILY_AI_TIMEOUT_MS`, defaulting to 30 minutes.

Daily AI can only become `complete` when the registered Browser Use CLI summary has `full_flow_completion.ok === true`, all required registered stages, cleanup proof, and workflow-owned source-of-truth readbacks. Recording/Gemini evidence can strengthen the run and must veto a contradictory completion claim, but it does not replace the workflow-owned registered summary or queue/artifact proof. A complete run records `daily_ai_publish`, `daily_ai_feed_study`, `daily_ai_engagement`, `daily_ai_sync`, `daily_ai_buffer`, `daily_ai_cleanup`, and `daily_ai_registered_summary` proofs. Partial/blocked runs record only the stages that actually exist; missing workflow-owned proofs stay in `proof_gate.missing`.

NisenPrints and other registered workflows use the same portable external worker contract. Browser Use CLI is the only browser execution surface; legacy runner names are compatibility identifiers and are fail-closed if their source is not a canonical Browser Use CLI adapter. Other registered runners may attach Gemini video QA as auxiliary visual audit evidence. Matching audits can appear as `gemini_video_qa` proof and contradictory audits can block a claimed completion, but Gemini QA does not replace workflow-owned proofs such as `pinterest_pin_url_verified` or `etsy_visit_site_match_verified`.
