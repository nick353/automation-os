...
The LaunchAgent template `ops/launchd/com.nichikatanaka.automation-os-worker.plist` is a relocatable template. `scripts/install-automation-os-worker-launch-agent.sh` materializes the current repository, helper, log, and profile paths and keeps database URLs and tokens out of the plist. Only install or kickstart it after `worker:production-proof:stored` passes; an invalid stored PostgreSQL secret is a hard startup blocker and must not be bypassed with SQLite or a Codex App bridge.
