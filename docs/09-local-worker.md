...
Worker receipts use the local Automation OS worker contract. The production stored-secret worker defaults to `AUTOMATION_OS_PORTABLE_WORKER_MODE=canary` for the portable registered workflows (求人応募管理, Daily AI, and NisenPrints): it validates the workflow manifest, binds the run, records a same-run readback receipt, and stops before Browser Use CLI, MCP, Playwright, Codex App, or any external effect. The exact blocker is `portable_external_effects_disabled`. This is intentional until each workflow has its own approved external-effect adapter and provider readback contract. `OPENAI_API_KEY` is not required.
