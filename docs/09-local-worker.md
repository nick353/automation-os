...
Worker receipts use the local Mac Codex App Server and the canonical Browser Use CLI lane for Automation OS browser work. Existing workflow-owned runners may remain Playwright or another explicitly registered contract until that workflow is migrated; their receipts are labeled as workflow-owned and are not treated as Browser Use CLI proof. `OPENAI_API_KEY` is not required.
