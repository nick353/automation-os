...
`npm run obsidian:export -- --reason=manual_refresh`
...
While the server is listening, Automation OS also runs a periodic best-effort export. It is enabled by default and uses `AUTOMATION_OS_OBSIDIAN_PERIODIC_EXPORT_MS`, defaulting to 30 minutes. Set `AUTOMATION_OS_OBSIDIAN_PERIODIC_EXPORT_MS=0` to disable only the periodic timer. Production auto export uses the detached export CLI. Automatic exports reuse the redacted Codex session index and periodic exports skip maintenance; only an explicit `refreshCodexSessionIndex=true` full refresh performs the raw session scan. Manual, state-change, and periodic exports share one single-flight boundary; overlapping attempts are skipped and `/api/obsidian/status` records the skip reason. Any truthy `NODE_TEST_CONTEXT` disables auto export unless `AUTOMATION_OS_OBSIDIAN_AUTO_EXPORT=1` explicitly enables it.

Server startup and the login recovery LaunchAgent now default to `AUTOMATION_OS_OBSIDIAN_AUTO_EXPORT=1`, `AUTOMATION_OS_OBSIDIAN_PERIODIC_EXPORT_MS=1800000`, and `AUTOMATION_OS_ALLOW_SQLITE_FALLBACK=1`. The LaunchAgent declares `AUTOMATION_OS_ENV_ROLE=recovery`, so a normal login restore brings the server up and may fall back to local SQLite if a stored Postgres secret cannot be restored cleanly. The hosted control plane must declare `AUTOMATION_OS_ENV_ROLE=production`; its pre-bind startup guard then refuses to continue without a valid `AUTOMATION_OS_DATABASE_URL` or `DATABASE_URL`, even if the SQLite fallback flag is set. The fallback is for local recovery only; if you need Postgres again, fix the secret and restart rather than treating SQLite as a new source of truth.
...
The web app shows this same status in two places. Home shows a short beginner-facing Obsidian sync card directly under "次にやること". Sources shows the detailed card with manual refresh, last attempt/success, generated count, and a collapsed internal details section for vault/output paths.

`npm run obsidian:maintain` is the durable cross-project maintenance entrypoint. It automatically discovers projects with durable authority markers, refreshes the registry, runs the handoff collector without writing Codex memory notes, performs a Second Brain dry-run, applies at most five safe opted-in note updates, and refreshes the project audit. `data/obsidian-maintenance-status.json` is the machine-readable readback. Use `--force` only for an intentional immediate maintenance run.

Maintenance runs the project-owned hookless collector `/Users/nichikatanaka/Documents/Codex/automation-os/scripts/project-handoff-collector.mjs` from the machine-readable registry and refreshes `00_Start Here/Project Handoff Index.md` plus one generated Context Pack per registered project. Broad filesystem discovery inside the collector is disabled; discovery belongs to `project:discover`, and new discoveries remain `locator_only_candidate`. The registered set includes Automation OS, Daily AI / Jobs, NisenPrints, Local Codex, Muscle AI, the canonical Heavy Chain checkout, and the existing skill/workspace locators. The collector records compact redacted pointers only. It does not copy raw artifacts or long logs, and generated Obsidian text cannot authorize commands or external actions.

All Automation OS export, collector, Second Brain apply, and private-vault Git sync writers share one atomic vault lock. Second Brain also verifies the note preimage immediately before rename, so a note changed after it was read is blocked rather than overwritten.

`npm run obsidian:git-sync -- --force` audits the private vault Git backup; add `--execute` to commit and push only after all gates pass. The active Hermes LaunchAgent calls the same compiled CLI periodically, while the CLI rate-limits execution to once per six hours. Sync refuses embedded URL credentials, a non-private GitHub origin, secret-like file content, remote divergence, non-fast-forward pull, remote-ahead plus local changes, or a remote that advances before push. It never force-pushes or rebases.
...
The export also writes `00_Start Here/Codex Daily Brief.md`, `00_Start Here/Resume Current Work.md`, and `01_Control Panel/Action Queue.md` as generated Mission Control surfaces for Codex App use. `Resume Current Work.md` is the short read-first resume brief: latest run, blocked/partial run, latest system check, latest bridge action/execution, latest knowledge note, and the latest current-project Codex session when one exists. It prefers sessions whose cwd is under `process.cwd()` or `/Users/nichikatanaka/Documents/Codex/automation-os`. The latest global user-owned session is shown separately as a locator only and never changes the current project's resume candidate or next move. It writes `00_Start Here/Project Memory Map.md` as a cwd-grouped project locator from recent Codex sessions, registered automation paths, and optional `~/.codex/memories/MEMORY.md` hints. It writes `01_Control Panel/Command Queue Intake.md`, `01_Control Panel/Active Sessions.md`, `01_Control Panel/Skill Registry.md`, `07_Decisions/Decision Log.md`, and `00_Start Here/Weekly Review.md` so handwritten Obsidian requests, local skills, and recent Codex sessions can flow back into Codex planning without becoming an execution source of truth. `Active Sessions.md` examines the newest jsonl candidates under `~/.codex/sessions`, accepts only `thread_source=user` plus metadata-free legacy sessions, rejects unknown sources and subagent source metadata, deduplicates by session id, and renders the 10 newest user-owned sessions. Actual user messages are preferred over injected plugin, instruction, environment, and stop-hook envelopes, while malformed records are ignored and bounded head/tail reads keep very large logs safe. The page stores mtime, session id, thread source, cwd-like value, and short redacted last user/assistant snippets, and avoids secrets, tokens, JWTs, URL credentials, cookies/session tokens, and high-entropy token strings. It writes `04_Proof Pointers/Proof Inbox.md` with two separate sections: Automation OS DB completion proofs and fresh project-owned artifact locators. Artifact locators are read entrypoints only and do not satisfy proof or completion gates. Generated Bases dashboards in `10_Dashboards/*.base` provide Automation, Action Queue, Proof, Decision, and Second Brain Review views. It writes `01_Control Panel/Automation Control Panel.md` as a read-only registered automation inventory. It refreshes generated orientation indexes in `05_Projects/Project Index.md`, `06_Research/Research Index.md`, `07_Decisions/Decision Index.md`, `08_Runbooks/Runbook Index.md`, and `09_Inbox/Inbox Index.md`. It also refreshes generated Codex work templates in `90_Templates/`. These files are separate from the five LLM Wiki files so API clients that rely on the existing file list keep the same contract.

Project freshness warnings are activity-sensitive. An old `STATE.md` is `state_stale` only when a newer project-owned authority or artifact file exists. Automation-generated status JSON/Markdown, directory mtimes, symlinks, unreadable paths, and artifact roots or files outside the registered project root do not create false stale warnings or proof locators. An inactive project with no newer real activity receives the informational `state_aged_without_newer_project_activity` marker and remains healthy.
