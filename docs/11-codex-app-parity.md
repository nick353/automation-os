...
- `00_Start Here/Resume Current Work.md` is the read-first Codex resume brief. It combines the latest run, blocked/partial run, latest system check, latest bridge action/execution, latest knowledge note, and the latest current-project Codex session summary into a short next-session handoff. The latest global user-owned session is displayed separately as a locator and cannot change the current project's resume priority or next move.
...
- `01_Control Panel/Active Sessions.md` lists the 10 newest deduplicated user-owned `~/.codex/sessions/**/*.jsonl` sessions with mtime, session id, thread source, cwd-like value, and short redacted last user/assistant snippets. Only `user` and metadata-free legacy sources are accepted; unknown, subagent, and automation sources plus injected plugin, instruction, environment, and stop-hook envelopes are excluded. Malformed and very large logs are handled with bounded reads. It is a locator, not a transcript export.
...
- `04_Proof Pointers/Proof Inbox.md` keeps Automation OS DB proofs separate from project-owned artifact locators. Locators help Codex fresh-read project evidence but never count as completion proof. `10_Dashboards/*.base` are generated by the same Obsidian export, so Bases dashboards stay current after state-changing Automation OS operations.
